...
def _process_more_for_video(provider, context):
    ...
    result = context.get_ui().on_select(context.localize(provider.LOCAL_MAP['youtube.video.more']), title=context.localize(provider.LOCAL_MAP['youtube.video.more']), items)
    if result != -1:
